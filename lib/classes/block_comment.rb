module FirstLogicTemplate
  ...
  def BlockComment.delete_all(ary)
    x = super(ary)
    ii = BlockComment.where('block_id = ?', ary[1]).order(:seq_id)
    ii.each_with_index {|i,j| i.update(seq_id:j+1) }
    x  # the number of deleted rows must be returned
  end

  def BlockComment.pop_c(block_id,j=1)
    ...
    raise ArgumentError,'# to pop larger than number of comments' if
        j > BlockComment.where('block_id = ?', block_id).count
    cc = BlockComment.where('block_id = ?', block_id).order(seq_id: :desc).limit(j)
    rtrn = []
    cc.each {|c|
      rtrn.unshift(c)
      BlockComment.delete(c.id)
    }
    rtrn
  end

  def set_seq_id(o)
    max = BlockComment.where('block_id = ?', o[:block_id]).maximum(:seq_id) || 0
    case
      when o[:seq_id].nil?
        o[:seq_id] = max + 1
